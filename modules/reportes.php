...
// KPIs del mes
$ventas_mes    = $db->prepare("SELECT COALESCE(SUM(total),0) FROM ventas WHERE estado='pagado' AND fecha BETWEEN ? AND ?"); $ventas_mes->execute([$inicio.' 00:00:00',$fin.' 23:59:59']); $ventas_mes=$ventas_mes->fetchColumn();
$ventas_prod   = $db->prepare("SELECT COALESCE(SUM(vi.subtotal),0) FROM venta_items vi JOIN ventas v ON v.id=vi.venta_id WHERE vi.tipo='producto' AND v.estado='pagado' AND v.fecha BETWEEN ? AND ?"); $ventas_prod->execute([$inicio.' 00:00:00',$fin.' 23:59:59']); $ventas_prod=$ventas_prod->fetchColumn();
$pacientes_mes = $db->prepare("SELECT COUNT(DISTINCT mascota_id) FROM consultas WHERE fecha BETWEEN ? AND ?"); $pacientes_mes->execute([$inicio.' 00:00:00',$fin.' 23:59:59']); $pacientes_mes=$pacientes_mes->fetchColumn();
$nuevos_cli    = $db->prepare("SELECT COUNT(*) FROM clientes WHERE created_at BETWEEN ? AND ?"); $nuevos_cli->execute([$inicio.' 00:00:00',$fin.' 23:59:59']); $nuevos_cli=$nuevos_cli->fetchColumn();

// Mes anterior para deltas
$mes_ant  = date('Y-m', strtotime($inicio.' -1 month'));
$ini_ant  = $mes_ant.'-01'; $fin_ant = date('Y-m-t',strtotime($ini_ant));
$vt_ant   = $db->prepare("SELECT COALESCE(SUM(total),0) FROM ventas WHERE estado='pagado' AND fecha BETWEEN ? AND ?"); $vt_ant->execute([$ini_ant.' 00:00:00',$fin_ant.' 23:59:59']); $vt_ant=$vt_ant->fetchColumn();
$delta_v  = $vt_ant>0 ? round(($ventas_mes-$vt_ant)/$vt_ant*100,1) : 0;

// Ventas diarias del mes
$ventas_diarias = $db->prepare("SELECT DATE(fecha) as dia, COALESCE(SUM(total),0) as total FROM ventas WHERE estado='pagado' AND fecha BETWEEN ? AND ? GROUP BY DATE(fecha) ORDER BY dia");
...
$servicios_top = $db->prepare("SELECT tipo, COUNT(*) as cnt FROM citas WHERE fecha BETWEEN ? AND ? GROUP BY tipo ORDER BY cnt DESC LIMIT 6"); $servicios_top->execute([$inicio,$fin]); $servicios_top=$servicios_top->fetchAll();
$max_srv = max(array_column($servicios_top,'cnt') ?: [1]);

// Productos más vendidos
$prods_top = $db->prepare("SELECT vi.descripcion, SUM(vi.cantidad) as qty, SUM(vi.subtotal) as total FROM venta_items vi JOIN ventas v ON v.id=vi.venta_id WHERE vi.tipo='producto' AND v.estado='pagado' AND v.fecha BETWEEN ? AND ? GROUP BY vi.referencia_id,vi.descripcion ORDER BY qty DESC LIMIT 8");
$prods_top->execute([$inicio.' 00:00:00',$fin.' 23:59:59']); $prods_top=$prods_top->fetchAll();

// Clientes más frecuentes
$clientes_top = $db->prepare("SELECT c.nombre,c.telefono,COUNT(DISTINCT con.id) as consultas, COALESCE(SUM(v.total),0) as gasto FROM clientes c LEFT JOIN mascotas m ON m.cliente_id=c.id LEFT JOIN consultas con ON con.mascota_id=m.id AND con.fecha BETWEEN ? AND ? LEFT JOIN ventas v ON v.cliente_id=c.id AND v.estado='pagado' AND v.fecha BETWEEN ? AND ? GROUP BY c.id ORDER BY consultas DESC, gasto DESC LIMIT 8");
...
$vets_stats = $db->prepare("SELECT u.nombre,COUNT(con.id) as atenciones FROM usuarios u LEFT JOIN consultas con ON con.veterinario_id=u.id AND con.fecha BETWEEN ? AND ? WHERE u.rol IN ('veterinario','admin') AND u.activo=1 GROUP BY u.id ORDER BY atenciones DESC");
...
for ($i=5;$i>=0;$i--) {
    ...
    $st=$db->prepare("SELECT COALESCE(SUM(total),0) FROM ventas WHERE estado='pagado' AND fecha BETWEEN ? AND ?");
